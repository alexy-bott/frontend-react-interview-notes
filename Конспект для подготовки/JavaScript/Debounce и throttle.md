...
В реальном коде нужно отдельно обработать сетевую ошибку и не показывать отмену как ошибку пользователю. Подробная модель находится в [[Конспект для подготовки/JavaScript/AbortController]].
