...
> [!faq]+ Уточнения
> - Flaky test проходит и падает без изменения кода из-за неопределённости.
> - Fixed sleep заменяют ожиданием конкретного UI/network состояния.
> - Каждый тест должен сам готовить и очищать данные.
> - Trace/video/screenshot/logs нужны для быстрой диагностики.
> - Retries маскируют симптом, но не устраняют причину flaky.
...
- Исправлять flaky увеличением timeout.
- Делать тесты зависимыми от порядка.
- Использовать общий пользовательский аккаунт во всех E2E.
- Не сбрасывать mocks, timers, storage и DOM между тестами.
- Выбирать элементы по CSS-классам, которые меняются от рефакторинга.
- Игнорировать редкие падения, пока они не станут постоянными.
