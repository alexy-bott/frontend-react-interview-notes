...
> [!faq]+ Уточнения
> - Условный рендер удаляет поддерево из React tree, Activity скрывает его и сохраняет внутренний state.
> - В `hidden` mode React очищает эффекты скрытого поддерева: подписки, timers, observers и другие side effects не продолжают жить как у видимой части.
> - Activity уместен для вкладок с формами, drawer, pre-render следующего экрана и быстрого возврата назад.
> - Если состояние сохранять не требуется, условный рендер обычно проще.
> - Если нужен только визуальный hide без React lifecycle-семантики, может хватить CSS.
