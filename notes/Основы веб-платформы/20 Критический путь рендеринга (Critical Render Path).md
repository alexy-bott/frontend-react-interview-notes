...
Детали layout, paint и composite разобраны отдельно в [Rendering pipeline reflow repaint composite](<../Устройство браузера/03 Конвейер рендеринга — reflow, repaint и composite.md>). В этой карточке важен путь первоначального документа до видимого результата.
